...
BEGIN
    ...
    SET @cNewDescription = 'Grid_LwStats_Command';

                        -- COMMENT is 150 Characters MAX
    SET @cNewComment = 'Add .grid lwstats GM diagnostic command for LivingWorld B-Cell envelope stats.';
    ...
    -- Add .grid lwstats diagnostic command for LivingWorld B-Cell envelope stats
    -- This GM-only, in-game-only command prints per-map cell-envelope counters
    -- (loads, accretions, fills, anomaly counts) for runtime verification.

    INSERT INTO `command` (`command_text`, `security`, `help_text`) VALUES
    ('grid lwstats',3,'Syntax: .grid lwstats\r\n\r\nPrints the per-map CellEnvelopeStats counters (envelopeLoads, accretions, fills, and anomaly counters) for the player''s current map. Used for LivingWorld B-Cell runtime verification.');
