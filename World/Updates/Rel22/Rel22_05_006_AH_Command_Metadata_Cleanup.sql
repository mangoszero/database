...
BEGIN
    ...
    SET @cNewDescription = 'AH_Command_Metadata_Cleanup';

                        -- COMMENT is 150 Characters MAX
    SET @cNewComment = 'Clean duplicate AH command metadata and keep only executable AH command rows.';
    ...
    WHERE `id` IN (
        SELECT `id`
        FROM `command`
        WHERE `command_text` IN ('ah', 'ah console', 'ah console show', 'ah console hide', 'ah repair')
    );
    ...
    SELECT `id`
    FROM `command`
    WHERE `command_text` IN ('ah console show', 'ah console hide', 'ah repair');
