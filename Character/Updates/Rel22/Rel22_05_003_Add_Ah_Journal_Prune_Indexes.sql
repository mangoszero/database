...
BEGIN
    ...
    SET @cNewDescription = 'Add_Ah_Journal_Prune_Indexes';

                        -- COMMENT is 150 Characters MAX
    SET @cNewComment = 'Add bounded worker journal retention indexes';
    ...
    IF (@cCurResult = @oldResult) THEN    -- Does the current version match the expected version
        ...
        IF NOT EXISTS (
            ...
            WHERE `table_schema` = DATABASE()
              AND `table_name` = 'ah_worker_journal'
              AND `index_name` = 'idx_state_created'
        ) THEN
            ALTER TABLE `ah_worker_journal`
                ADD KEY `idx_state_created` (`state`, `created_time`);
        END IF;
        ...
        IF NOT EXISTS (
            ...
            WHERE `table_schema` = DATABASE()
              AND `table_name` = 'ah_worker_journal'
              AND `index_name` = 'idx_state_resolved'
        ) THEN
            ALTER TABLE `ah_worker_journal`
                ADD KEY `idx_state_resolved` (`state`, `resolved_time`);
        END IF;
        ...
        IF EXISTS (
            ...
            ALTER TABLE `ah_worker_journal` DROP INDEX `idx_state`;
        END IF;
